...
### Added — Loop tool
- **Video Loop** — perpanjang clip pendek jadi video panjang (30 menit - 24 jam) untuk 24/7 livestream. Accessible via sidebar "Loop" di bawah Streams. Pilih video sumber, target durasi (preset: 30m/1h/2h/3h/6h/12h/24h + custom menit), optional custom title. FFmpeg pakai `-stream_loop -1 -i <src> -c copy -t <target>` — tidak re-encode, jadi cepat sekali (beberapa detik untuk output 3 jam dari clip 10 detik). Output jadi row video baru di Library dengan status `transcoding` → `ready`, auto-thumbnail generation, inherit folder & resolution dari source. Live progress bar + ETA + speed factor via polling `/looper/progress` JSON tiap 2 detik. Cancel job mid-run tersedia. Module baru `src/looper.js` dengan pattern mirip `transcoder.js` (Map jobs, progress events dari `-progress pipe:1`). Route `/looper` + `/looper/start` + `/looper/progress` + `/looper/:jobId/cancel`. View `views/looper.ejs`. Catatan: untuk clip AI-generated (Veo, Runway, dll), fireplace/ambient visuals yang repetitif bisa di-loop tanpa crossfade karena mata tidak notice boundary — otak viewer tidak ingat frame persisnya.
